...
export const MapHome: React.FC<YandexMapProps> = ({ filteredRoutes, onPointClick, onMapClick }) => {
  ...
  const handlePlacemarkClick = (point: { latitude: number; longitude: number }, route: Route) => {
      ...
      >
        ...
        {points.map((point) => (
          <Placemark
            ...
            options={{
              preset: selectedPoint && selectedPoint.latitude === point.latitude && selectedPoint.longitude === point.longitude
                ? 'islands#greenIcon'  // Зеленый маркер для выбранной точки
                : 'islands#blueIcon',  // Синий маркер для остальных точек
            }}
            onClick={() => handlePlacemarkClick(point, filteredRoutes.find(route => route.points[0] === point)!)}  // Находим маршрут по первой точке
          />
        ))}
